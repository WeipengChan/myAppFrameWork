...
#define kContentMinOffset           65
#define kMoveAnimationDuration      0.1f
...
//中间View的阴影的添加
    ...
    _contentViewController.view.layer.shadowOffset = CGSizeMake(2.5, 2.0);
    _contentViewController.view.layer.shadowRadius =  2.0;
     _contentViewController.view.layer.shadowOpacity =  0.5;
    _contentViewController.view.layer.shadowColor = [UIColor grayColor].CGColor;
    _contentViewController.view.layer.shadowPath = [UIBezierPath bezierPathWithRect: _contentViewController.view.layer.bounds].CGPath;
...
- (void)goToNewDiection:(SideBarShowDirection)direction forceChanged:(BOOL)bForce
{
    ...
    switch (self.currentDirection)
    {
        case SideBarShowDirectionNone:
        {
            ...
            self.contentViewController.currentScrollView.scrollEnabled = YES;
        ...
        {
            self.contentViewController.view.transform  = CGAffineTransformMakeTranslation(kContentOffset - 140, 0);
            
            self.contentViewController.currentScrollView.scrollEnabled = NO;
...
- (void)panInContentView:(UIPanGestureRecognizer *)panGestureReconginzer
{
	if (panGestureReconginzer.state == UIGestureRecognizerStateChanged)
    {
       ...
       // MSDebug(@"translation-- %f ",translation);
        if ((_currentTranslate == 0  && translation <0)  || (_currentTranslate == 135 && translation > 0) ) {
    ...
    {
		_currentTranslate = self.contentViewController.view.transform.tx;
      //  MSDebug(@"_currentTranslate-- %f ",_currentTranslate);
        if (!_sideBarShowing)
        {
